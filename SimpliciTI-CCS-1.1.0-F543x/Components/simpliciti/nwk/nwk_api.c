/**************************************************************************************************
  ...
  WITHOUT WARRANTY OF ANY KIND, EITHER EXPRESS OR IMPLIED, INCLUDING WITHOU1T LIMITATION, ANY
...
/**************************************************************************************
 * Added by Ghaith to get the source address and port number from the received frame
 * @fn          SMPL_Receive
 *
 * @brief       Receive a message from a peer application.
 *
 ...
 *              SMPL_NO_CHANNEL     Frequency Agility enabled and could not find channel
 */
smplStatus_t SMPL_Receive_Src(linkID_t lid, uint8_t *msg, uint8_t *len, addr_t *sourceAdd, rssi_t *RSSI , uint8_t *LQI, uint8_t *tractID)
{
  connInfo_t  *pCInfo = nwk_getConnInfo(lid);
  smplStatus_t rc = SMPL_BAD_PARAM;
  rcvContext_t rcv;
  //connInfo_t  *pCInfo = 0; //added by Ghaith
  if (!pCInfo || ((rc=nwk_checkConnInfo(pCInfo, CHK_RX)) != SMPL_SUCCESS))
  {
    return rc;
  }
//  else//check if the packet is broadcast
//  {
//  	
//  }
...
//Ghaith: Add code to get the port number from received message

 //   port   = &(pCInfo->portRx);
//    pAddr2 = pCInfo->peerAddr;
  
 //    port = pCInfo->peerAddr;
//Ghaith:  the forth argument of the nwk_retrieveFrame(..) is the SrcAddress. Add code
// here to get the source address from the received frame.     
  /* TODO: deal with pending */    
      rc = nwk_retrieveFrame(&rcv, msg, len, sourceAdd,0, RSSI, LQI,tractID);
...
//	smplStatus_t status;
//	 status = 
//	RSSI = &(pCInfo->sigInfo.rssi);
//	LQI = &(pCInfo->sigInfo.lqi);
 return nwk_retrieveFrame_ConnInfo(&rcv, msg, len, sourceAdd, 0, RSSI, LQI,tractID);
 //return nwk_retrieveFrame(&rcv, msg, len, sourceAdd, 0);
#endif  /* RX_POLLS */
}
/******************************************************************************
 * @fn          SMPL_Link
 ...
 * @fn          SMPL_Bcast
 *
 * @brief       Broadcast a message to all peer applications.
 *
 * input parameters
 * 
 *
 ...
 *                               and no acknowledgement is received
 */
smplStatus_t SMPL_Bcast(uint8_t *msg, uint8_t len)
{
  frameInfo_t  *pFrameInfo;
  //connInfo_t   *pCInfo     = nwk_getConnInfo(lid);
  smplStatus_t  rc         = SMPL_BAD_PARAM;
  uint8_t       radioState = MRFI_GetRadioState();
  uint8_t       ackreq     = 0;
 // uint8_t port = 0x30; // broadcast port ... any port doesn't really matter 
  addr_t *bcast_address;
 ...
 // pFrameInfo = nwk_buildFrame(SMPL_PORT_USER_BCAST, msg, len, 2);
  pFrameInfo = nwk_buildFrame(0x20, msg, len, 1);
...
//    if (SMPL_LINKID_USER_UUD != lid)
//    {
//      pFrameInfo = nwk_buildAckReqFrame(port, msg, len,hops , 0xFF);
//      ackreq     = 1;
//    }
//    else
//    {
...
//#endif  /* APP_AUTO_ACK */
  ...
  bcast_address = (addr_t *)nwk_getBCastAddress();
  memcpy(MRFI_P_DST_ADDR(&pFrameInfo->mrfiPkt), bcast_address, NET_ADDR_SIZE);
 ...
 * @fn          SMPL_SendDst
 *
 * @brief       Send a message using destination address anf port number.
 *
 * input parameters
 ...
 *                               and no acknowledgement is received
 */
smplStatus_t SMPL_SendDst(addr_t *dstAddress, uint8_t port, uint8_t *msg, uint8_t len)
{
  frameInfo_t  *pFrameInfo;
  smplStatus_t  rc = SMPL_BAD_PARAM;
  uint8_t radioState = MRFI_GetRadioState();
   ...
   * connection info.
   */
   pFrameInfo = nwk_buildFrame(port, msg, len, 1);
  ...
  memcpy(MRFI_P_DST_ADDR(&pFrameInfo->mrfiPkt), dstAddress, NET_ADDR_SIZE);
