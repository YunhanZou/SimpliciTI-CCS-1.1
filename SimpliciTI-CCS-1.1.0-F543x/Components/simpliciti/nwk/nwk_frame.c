...
/******************************************************************************
 * Added by Ghaith to get Port number from Frame and hand it to SMPL_Receive_Src()
 * @fn          nwk_retrieveFrame
 *
 * @brief       Retrieve frame from Rx frame queue. Invoked by application-level
 *              code either app through SMPL_Receive() or IOCTL through raw Rx. This
 ...
 *
 */
smplStatus_t nwk_retrieveFrame_ConnInfo(rcvContext_t *rcv, uint8_t *msg, uint8_t *len, addr_t *srcAddr, uint8_t *hopCount,rssi_t *RSSI, uint8_t *LQI, uint8_t *tractID)
{
  frameInfo_t *fPtr;
  uint8_t      done;
    ...
    if (fPtr)
    {
      ...
      *len = MRFI_GET_PAYLOAD_LEN(&fPtr->mrfiPkt) - F_APP_PAYLOAD_OS;
      ...
      if (pCInfo)
      {
        ...
        pCInfo->sigInfo.rssi = fPtr->mrfiPkt.rxMetrics[MRFI_RX_METRICS_RSSI_OFS];
        if(RSSI){
        memcpy(RSSI, &pCInfo->sigInfo.rssi, 1); //line added by Ghaith
        }
      //  pCInfo->sigInfo.lqi  = fPtr->mrfiPkt.rxMetrics[MRFI_RX_METRICS_RSSI_OFS];
        pCInfo->sigInfo.lqi  = fPtr->mrfiPkt.rxMetrics[MRFI_RX_METRICS_CRC_LQI_OFS];
        if(LQI){
        memcpy(LQI, &pCInfo->sigInfo.lqi, 1); //line added by Ghaith
        }
      }
      ...
      if(tractID) //added by Ghaith to transaction ID
      {
      	*tractID = GET_FROM_FRAME(MRFI_P_PAYLOAD(&fPtr->mrfiPkt),F_TRACTID_OS);
      }
      /* input frame no longer needed. free it. */
      nwk_QadjustOrder(INQ, fPtr->orderStamp);
