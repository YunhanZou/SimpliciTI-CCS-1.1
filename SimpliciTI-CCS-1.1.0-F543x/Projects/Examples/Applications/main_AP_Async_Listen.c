...
static void processMessage(linkID_t lid, uint8_t *msg, uint8_t len, addr_t *sAdd, rssi_t *rssi, uint8_t* lqi, uint8_t* tractID);
  ...
  while (1)
  {
      ...
      while (1)
      {
        if (SMPL_SUCCESS == SMPL_LinkListen(&sLID[sNumCurrentPeers]))
        {
...
#endif          
          {
		    ...
		    {
              if (SMPL_SUCCESS == SMPL_Send(sLID[sNumCurrentPeers], cmd, 2))
     		  {
		   	    break;
       		  }
        	}
          ...
          else
          {
		    ...
		    for(i=0;i<3;i++)  // 3 attempts to send command
		    {
              if (SMPL_SUCCESS == SMPL_Send(sLID[sNumCurrentPeers], cmd, 4))
     		  {
		   	    break;
       		  }
        	}
    ...
    if (sPeerFrameSem)
    {
      ...
      for (i=0; i<sNumCurrentPeers; ++i)
      {
        ...
        {
          processMessage(sLID[i], msg, len, &sAdd, &rcRSSI, &rcLQI, &tractID);
